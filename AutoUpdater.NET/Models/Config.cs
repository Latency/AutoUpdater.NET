...
internal sealed partial class Config : ObservableObject, IConfig, ICloneable
{
    ...
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnWindowSizeOverrideChanged(bool value) => _UpdateValidation();
    ...
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnIsIconOverrideChanged(bool value) => _UpdateValidation();

    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    [ObservableProperty]
    public partial IconOverride? IconOverride { get; set; }
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnIconOverrideChanged(IconOverride? value) => _UpdateValidation();
    ...
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnProxyEnabledChanged(bool value) => _UpdateValidation();

    [JsonIgnore]
    [ObservableProperty]
    public partial string? ProxyUri { get; set; }
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnProxyUriChanged(string? value) => _UpdateValidation();

    [JsonIgnore]
    [ObservableProperty]
    public partial string? ProxyUserName { get; set; }
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnProxyUserNameChanged(string? value) => _UpdateValidation();

    [JsonIgnore]
    [ObservableProperty]
    public partial string? ProxyPassword { get; set; }
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnProxyPasswordChanged(string? value) => _UpdateValidation();

    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    [ObservableProperty]
    public partial ProxyEnabled? Proxy { get; set; }
    // ReSharper disable once UnusedParameterInPartialMethod
    partial void OnProxyChanged(ProxyEnabled? value) => _UpdateValidation();
    ...
    [JsonIgnore]
    [ObservableProperty]
    public partial BitmapImage? TmpIcon { get; set; }
    // ReSharper disable once UnusedParameterInPartialMethod
    ...
    internal void _UpdateVersion() => UpdateVersion?.Invoke($"Loader Version: {InstalledVersion}");

    internal void _UpdateIcon(BitmapImage? value) => UpdateIcon?.Invoke(value);
    ...
    internal void Clone(IConfig other)
    {
        foreach (var prop in other.GetType().GetProperties())
        {
            if (prop.PropertyType.IsValueType || prop.PropertyType == typeof(string) || prop.PropertyType == typeof(BitmapImage) || prop.PropertyType == typeof(ObservableCollection<TreeViewItem>))
            {
                ...
                if (prop.Name == "IconOverride")
                {
                    if (TmpIcon is not null)
                        otherValue ??= new IconOverride
                        {
                            Uri = TmpIcon.UriSource
                        };
                }
                GetType().GetProperty(prop.Name)?.SetValue(this, otherValue);
            }
            else
            {
                ...
                {
                    not null when type == typeof(Version2) => (Version2)Assembly.GetExecutingAssembly().Version()!,
                    _                                      => ctor.Invoke([prop.GetValue(other)])
                };

                GetType().GetProperty(prop.Name)?.SetValue(this, instance);
                var value = GetType().GetProperty(prop.Name)?.GetValue(this);
                if (value is ObservableObject v)
                    v.PropertyChanged += (_, _) => _UpdateValidation();
            }
        }
    ...
    public bool Equals(IConfig? other)
    {
        ...
        bool IsFtpProtocol()
        {
            ...
            if (FtpProfile is not null && other.FtpProfile is not null)
            {
                ...
                bool IsEncoding() => FtpProfile.Encoding is not null && other.FtpProfile.Encoding is not null ? FtpProfile.Encoding.Equals(other.FtpProfile.Encoding) : FtpProfile.Encoding is null && other.FtpProfile.Encoding is null;
